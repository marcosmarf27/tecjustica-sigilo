...
export function PainelDenyList({ buscar, gravar, avisar }: PainelDenyListProps) {
  ...
  const pares = useMemo(() => {
    ...
    const filtrados = q ? todos.filter((p) => p.termo.toLowerCase().includes(q)) : todos;
  ...
  const remover = async (tipo: string, termo: string) => {
    if (!lista) return;
    setGravando(true);
    const nova = { ...lista, [tipo]: lista[tipo].filter((t) => t !== termo) };
  ...
  return (
    <LinhaDeAjuste
      titulo={`${total} termo${total === 1 ? "" : "s"} liberado${total === 1 ? "" : "s"}`}
      descricao="Palavras que o detector deixa passar — as que você marcou como “não é dado pessoal” na revisão, mais as expressões jurídicas que vêm de fábrica. Remover um termo faz o detector voltar a mascará-lo nos próximos documentos; os já processados não mudam."
      empilhado
    >
      {lista === null ? (
